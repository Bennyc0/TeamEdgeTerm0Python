    ...
    def move_selection(self):
        ...
        while active:
            if "sword" in player.items:
                move_type = input(
                    "Do you wish to use sword attacks or melee attacks?>> ")
            ...
            if self.block_check():
                ...
                while active:
                    ...
                    if player_guess.isdigit():
                        ...
                        elif int(player_guess) >= block_chance - 4 and int(
                                player_guess) <= block_chance + 4:
                            block_percent = 5
                            perfect_block = False
                            print("\n --<Good Block!>-- \n")
                            active = False

                        elif int(player_guess) >= block_chance - 7 and int(
                                player_guess) <= block_chance + 7:
                            block_percent = 3
                            print(
                                f"{self.name} uses {self.move}. {self.name} is ready to block some damage. \n"
                            )
                            active = False
                ...
                if self.damage > 0:
                    enemy.health = round(enemy.health - self.damage)
                    print(
                        f"{self.name} used {self.move} and dealt {self.damage} damage to {enemy.name}!"
                    )

                else:
                    print(
                        f"{self.name} used {self.move}. The {self.move} missed!"
                    )
            ...
            if self.is_alive() and enemy.is_alive():
                if select_move == "block" or select_move == "sword block":
                    if block_percent == 100:
                        print(
                            f"{self.name} excuted a Perfect Block! {self.name} took no damage. The {enemy.name} needs time to recover."
                        )
                        print(
                            f"{self.name}'s hp: {self.health} \n{enemy.name}'s hp: {enemy.health} \n"
                        )
                        use_hp_pot = input(
                            "Do you want to take this chance to use a hp potion?>> "
                        )
                        ...
                        self.health = round(self.health -
                                            enemy.damage / block_percent)
                        print(
                            f"The {enemy.name}'s {enemy_move} did {round(enemy.damage/block_percent)} damage."
                        )
                    else:
                        self.health = round(self.health -
                                            enemy.damage / block_percent)
                        print(
                            f"{self.name} used {self.move}! The {enemy.name}'s {enemy_move} did {round(enemy.damage/block_percent)} damage."
                        )
                        print(
                            f"{self.name}'s hp: {self.health} \n{enemy.name}'s hp: {enemy.health} \n"
                        )
                    ...
                    print(
                        f"{enemy.name} {enemy_move} {self.name} for {enemy.damage} damage!"
                    )
                    print(
                        f"{self.name}'s hp: {self.health} \n{enemy.name}'s hp: {enemy.health} \n"
                    )
...
# ----------The Database----------
# ---npcs---
west_gate_guards = NPC(
    "West Gate Guards",
    "Two armoured guards stand guard with their spears at the gate.",
    "Guard: Where are ya goin kiddo? Your too young to go into the forest. Go home now."
)
south_gate_guards = NPC(
    "South Gate Guards",
    "Two armoured guards stand guard with their spears at the gate.",
    "Guard: This is area is closed due to landslides in the area. It will take a few days before it's open again."
)
north_gate_guards = NPC(
    "North Gate Guards",
    "Two armoured guards stand guard with their spears at the gate.",
    "Guard: The scorching desert isn't a place for a kid like you. It is too extreme for you."
)
east_gate_guards = NPC(
    "East Gate Guards",
    "Two armoured guards stand guard with their spears at the gate.",
    "Guard: There has been bandit sightings recently. Take care out there!")
citizen1 = NPC(
    "Worried Lady", "A lady that has a very worried expression on her face.",
    "Lady: Theres a rumor going around that bandits are going to attack the city. I hope it's not true..."
)
citizen2 = NPC(
    "Busy Merchant", "A merchant unloading his boxes of wares from his cart.",
    "Merchant: Quit blocking my path! I gotta get these boxes unloaded.")
blacksmith = NPC(
    "Working Blacksmith",
    "A blacksmith hammering away on his unfinished sword.",
    "Blacksmith: Hm? What are you doing here? Are you here for the sword?")
sheep = NPC(
    "A Sheep", "A sheep eating its grass and enjoying the weather",
    "Sheep: BAAAAAAAA! \n(It's trying to tell you to leave it alone.)")
traveler = NPC(
    "Traveler", "A heavily wounded traveler.",
    "Traveler: TAKE this letter and GIVE it to the head knight in the city for me please. I won't be able to make it with these injuries. Also, whatever you do, don't go into the forest. \n--<Type 'take letter' to take the letter>--"
)
head_knight = NPC("Head Knight", "The head of the knights of the city.",
                  "Head Knight: How can I help you?")
...
# ---bandits---
bandit_alone = Enemy("Bandit", "--<A bandit with a cloak appeared>--",
                     "Bandit: MWAHAHAHAHA- DIE!!!", 100, random.randint(5, 10))
bandit_pair = Enemy("Bandit Pair", "--<2 wild bandits appeared!>--",
                    "Bandits: HAND OVER YOUR MONEY NOW!",
                    random.randint(40, 75), random.randint(5, 15))
bandit_group = Enemy("Bandit Group", "--<A group of wild bandits appeared!>--",
                     "Bandits: Take your pick, life or money?",
                     random.randint(80, 120), random.randint(10, 20))
bandit_army = Enemy("Bandit Army", "--<An army of wild bandits appeared!>--",
                    "DIE INTRUDER!", 250, 25)
bandit_leader = Enemy("Bandit Leader",
                      "--<The boss of the wild bandits has appeared!>--",
                      "You shouldn't had come here kid...", 500, 50)
...
punch = Player_attack("Punch", "The most basic of the basic melee attacks.",
                      random.randint(5, 10), 100)
uppercut = Player_attack("Uppercut", "A low punch that goes upwards.",
...
sword_attacks_list = [
    "sword pierce", "heavy slash", "sword slice", "sword block"
]
melee_attacks_list = [
    "down chop", "low kick", "punch", "roundhouse kick", "uppercut", "block"
]
...
# ---areas---
city_square = Area("City Square", "The center of the city.",
                   ["citizen1", "citizen2", "head knight"], [], [
                       "city smithy", "city west gate", "city south gate",
                       "city north gate", "city east gate"
                   ])
city_smithy = Area("City Smithy", "Your local blacksmith.", ["blacksmith"], [],
                   ["city square"])
city_west_gate = Area("City West Gate", "The west gate of the city.",
                      ["west gate guards"], [], ["city square"])
city_south_gate = Area("City South Gate", "The south gate of the city.",
                       ["south gate guards"], [], ["city square"])
city_north_gate = Area("City North Gate", "The north gate of the city.",
                       ["north gate guards"], [], ["city square"])
city_east_gate = Area("City East Gate", "The east gate of the city.",
                      ["east gate guards"], [],
                      ["city square", "grassy fields"])
grassy_fields = Area(
    "Grassy Fields ",
    "The lush green fields to the east of the city. There's a fork in the path leading to north and south Redwood Forest.",
    ["sheep"], [],
    ["city east gate", "grassy fields northeast", "grassy fields southeast"])
grassy_fields_northeast = Area(
    "Grassy Fields Northeast",
    "The path that goes to the north of the Redwood Forest.", ["traveler"],
    ["bandit pair"], ["redwood forest north", "grassy fields"])
grassy_fields_southeast = Area(
    "Grassy Fields Southeast",
    "The path that goes to the south of the Redwood Forest.", [],
    ["bandit group"], ["grassy fields", "redwood forest south"])
redwood_forest_north = Area("Redwood Forest North",
                            "Northern parts of the Redwood Forest.", [],
                            ["bandit army", "bandit leader"], [])
redwood_forest_south = Area(
    "Redwood Forest South", "Southern parts of the Redwood Forest.", [], [],
    ["grassy fields southeast", "deeper redwood forest"])
bandit_camp = Area("Deeper Redwood Forest",
                   "The camp bandits had made in the Redwood Forest.", [],
                   ["bandit army", "bandit leader"], [])
...
scorching_desert = Area(
    "Scorching Desert",
    "A desert north of the city that is filled with dangerous Sandworms and other monsters. The temperature can reach 110 degrees during daytime.",
    "", "", "")
azure_peak_pass = Area(
    "Azure Peak Pass",
    "The route through the Froxen Mountain Range. A landslide just happened recently.",
    "", "", "")
dark_oak_woods = Area(
    "Dark Oak Woods",
    "A forest filled with the dangerous Silver Wolves and Crimson Bears.", "",
    "", "")
...
def start():
    ...
    print("--<Hello Player>-- \n")
    print(
        "This is your last few days of in this city. After this week, your summer vacation ends and you have to go back home for school. \n\nYou decide that you want to explore the Redwood forest that is to the east of the city. \n"
    )
    ...
    while active:
        ...
        if player_name.strip():
            ...
            sleep(delay)
            confirm = input(
                f"Name accepted! Your name is {player_name}, correct?(Yes or No)>> "
            ).lower()
...
def player_command(input):
    ...
    if "go" in input:
        if len(input) >= 2:
            if len(current_area.enemies) == 0:
                ...
                print(
                    "--<Please fight the enemies in the area first!(If there is an enemy in the [], also don't include the [] or the '')>--"
                )
        else:
    ...
    # "look"
            ...
            for npc in current_area.npcs:
                if npc1 == npc or npc2 == npc or npc3 == npc:
                    ...
                    if npc == "head knight" and "letter" in player.items and current_area == city_square:
                        print(
                            "--<Type 'give letter' to give the letter to head knight!>--"
                        )

        else:
            print(
                "--<Please state the npc you want to talk with after 'talk'>--"
            )
        ...
        if len(input) >= 2:
            ...
            print(
                "--<Please state the enemy you want to fight after 'fight'>--")
    ...
    #extra features:
        ...
        print(
            "Blacksmith: Ok, hang on for a bit, im almost done with the sword. \n--<15 minutes later>-- \n"
        )
        print(f"--<{player.name} obtained a sword!>--")

    elif input == ['take', 'letter'
                   ] and current_area == grassy_fields_northeast:
        ...
        print(
            "--<You have successfully delivered the letter to the head knight>-- \n"
        )
        completion = "True"
    ...
    if current_area == bandit_camp or current_area == redwood_forest_north:
        ...
        print(
            "--<You prepare to fight them, knowing that you won't make it out alive>-- \n"
        )
        for enemy in current_area.enemies:
            target = enemy_dict.get(enemy)
            fight(player, target)
